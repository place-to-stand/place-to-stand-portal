...
CREATE OR REPLACE FUNCTION time_log_task_matches_project(p_time_log_id uuid, p_task_id uuid)
RETURNS boolean
LANGUAGE plpgsql
STABLE
AS $$
DECLARE
  time_log_project_id uuid;
  task_project_id uuid;
BEGIN
  SELECT project_id INTO time_log_project_id FROM time_logs WHERE id = p_time_log_id;
  SELECT project_id INTO task_project_id FROM tasks WHERE id = p_task_id;
  RETURN time_log_project_id = task_project_id;
END;
$$;--> statement-breakpoint
ALTER TABLE "time_log_tasks" ADD CONSTRAINT "time_log_tasks_project_match" CHECK (time_log_task_matches_project(time_log_id, task_id));
