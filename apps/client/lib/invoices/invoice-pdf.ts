/**
 * Copied verbatim from `apps/internal/lib/invoices/invoice-pdf.ts` — only the
 * type import path differs, so `diff` between the two files should show exactly
 * one line.
 *
 * If you change the invoice layout there, change it here too: clients read this
 * copy in the portal and that copy as the PDF attached to their invoice email,
 * and the two disagreeing is the failure mode this note exists to prevent.
 *
 * Promote both to a `packages/invoices` workspace package (modelled on
 * `@pts/github`) if the layout starts changing often.
 */
