...
export type OutboundEmail = {
  ...
  html: string
  /**
   * Resend deduplicates sends that share a key for 24 hours. Pass one when a
   * caller may legitimately retry after a lost response, so the retry cannot
   * become a second copy. Ignored by Mailpit.
   */
