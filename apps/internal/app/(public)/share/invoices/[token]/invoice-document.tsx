...
export function InvoiceDocument({ invoice, isPaid, isVoid }: Props) {
  ...
  return (
    <article
      className={cn(
        'bg-email-paper text-email-ink relative px-6 pt-7 pb-6 sm:px-14 sm:pt-12 sm:pb-10',
        isVoid && 'opacity-60'
      )}
    >
      <BlueprintCorners size={16} className='border-brand-lime' />
      ...
      <div className='mt-4 flex justify-end sm:mt-5'>
        <div className='flex w-full flex-col gap-2.5 sm:w-[280px]'>
          ...
          {/* Lime stays on a dark ground: the total sits in a brand-bg band. */}
          <div className='bg-brand-bg text-brand-text mt-1 flex items-baseline justify-between px-3.5 py-3'>
            <span className='text-brand-lime font-mono text-[11px] tracking-[0.1em] uppercase'>
              {totalLabel}
            </span>
            <span
              className={cn(
      ...
      {invoice.notes ? (
        <div className='mt-7 flex flex-col gap-2 sm:mt-10 sm:gap-2.5'>
          ...
          <p className='text-email-ink max-w-[520px] text-[13px] leading-[1.55] whitespace-pre-wrap sm:text-sm'>
            {invoice.notes}
          </p>
        </div>
      ) : null}

      <div className='border-email-rule text-email-muted mt-7 flex justify-between border-t pt-3 font-mono text-[10px] tracking-[0.1em] uppercase sm:mt-12 sm:pt-3.5 sm:text-[11px]'>
...
function MetaField({ label, value }: { label: string; value: string }) {
  return (
    <div className='border-email-rule first:border-email-ink sm:border-email-ink flex items-baseline justify-between border-t py-2.5 first:border-t-[1.5px] sm:flex-col sm:items-start sm:justify-start sm:gap-1.5 sm:border-t-[1.5px] sm:pt-3 sm:pb-0'>
...
function TotalsRow({ label, value }: { label: string; value: string }) {
  return (
    <div className='text-email-muted flex justify-between text-sm'>
      <span>{label}</span>
  ...
  return (
    <div
      className={cn(
        ...
        tone === 'paid'
          ? 'border-email-accent-frame text-email-accent-ink'
