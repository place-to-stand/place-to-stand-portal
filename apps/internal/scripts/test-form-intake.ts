...
const QUESTIONS = [
  ...
  {
    questionId: 'bottlenecks',
    sectionId: 'challenges',
    prompt: 'Where do things bog down?',
  },
  {
    questionId: 'budget',
    sectionId: 'readiness',
...
const RESULT = {
  ...
  recommendations: [
    ...
    },
    {
      ...
      tagline: 'Connect your tools and automate handoffs.',
      score: 9,
      reasons: ['Repetitive handoffs'],
    },
  ],
}
...
async function main() {
  ...
  for (const beacon of [
    ...
    auditBeacon({
      ...
      lead: {
        name: 'Dana Reed',
        email: `dana+${sessionId.slice(0, 8)}@example.com`,
        company: 'Acme Co.',
        message: 'We run everything on spreadsheets.',
  ...
  check(
    'top service denormalized to highest score',
    captured?.topServiceId === 'workflow-automation',
    captured?.topServiceId
  )
  check('phase denormalized', captured?.phaseId === 'growth', captured?.phaseId)
  check(
    'contact captured',
    captured?.contactName === 'Dana Reed',
    captured?.contactName
  )
  check(
    'lead message stored',
    captured?.message === 'We run everything on spreadsheets.',
    captured?.message
  )
  ...
  check(
    'status still captured',
    afterStale?.status === 'captured',
    afterStale?.status
  )
  check('result not nulled', afterStale?.result !== null)
  check(
    'contact not nulled',
    afterStale?.contactName === 'Dana Reed',
    afterStale?.contactName
  )
  check(
    'last_activity_at did not go backwards',
    afterStale?.lastActivityAt === captured?.lastActivityAt,
    afterStale?.lastActivityAt
  )
  check(
    'transcript not truncated',
    Array.isArray(afterStale?.responses) &&
      (afterStale?.responses as unknown[]).length === 4
  )

  console.log('\nNewer pagehide beacon (newer updatedAt, lower status)')
  const newer = await submitAudit(
    auditBeacon({
      status: 'abandoned',
      trigger: 'pagehide',
  ...
  check(
    'status STILL captured (GREATEST, not setWhere)',
    afterNewer?.status === 'captured',
    afterNewer?.status
  )
  check('result still preserved', afterNewer?.result !== null)
  check(
    'last_activity_at advanced',
    afterNewer?.lastActivityAt !== afterStale?.lastActivityAt
  )
  check(
    'duration advanced to 45s',
    afterNewer?.durationMs === 45_000,
    afterNewer?.durationMs
  )
  check(
    'captured_at unchanged (first capture wins)',
    afterNewer?.capturedAt === capturedAtFirstCapture,
    afterNewer?.capturedAt
  )
  ...
  check(
    'status is captured',
    contactRow?.status === 'captured',
    contactRow?.status
  )
  check(
    'subject stored',
    contactRow?.subject === 'Referral Program',
    contactRow?.subject
  )
  check(
    'message stored',
    contactRow?.message === 'Interested in a workflow audit.'
  )
  check(
    'attribution stored',
    contactRow?.utmCampaign === 'ai-audit-q3',
    contactRow?.utmCampaign
  )
  check(
    'audit columns null',
    contactRow?.responses === null && contactRow?.percentComplete === null
  )
  ...
  check(
    'retry -> 200 (idempotent, no duplicate)',
    retry.status === 200,
    retry.status
  )
  ...
  if (WITH_DELIVERY) {
    ...
    check(
      'no deliver -> emails skipped',
      quiet.body?.data?.emails?.team === 'skipped',
      quiet.body
    )
    ...
    check(
      'delivery_requested_at stamped',
      Boolean(firstRow?.deliveryRequestedAt)
    )
    ...
    check(
      'second submission -> sent',
      third.body?.data?.emails?.team === 'sent',
      third.body?.data
    )
    const fourth = await submitContact(contactBody(fourthId, true))
    check(
      'third submission -> sent (replay did not spend quota)',
      fourth.body?.data?.emails?.team === 'sent',
      fourth.body?.data
    )
    const fifth = await submitContact(contactBody(fifthId, true))
    check(
      'fourth submission -> 200 (still recorded)',
      fifth.status === 200,
      fifth.status
    )
    check(
      'fourth submission -> skipped',
      fifth.body?.data?.emails?.team === 'skipped',
      fifth.body?.data
    )
    const fifthRow = await readRow(fifthId)
    check(
      'fourth submission -> delivery not requested',
      fifthRow?.deliveryRequestedAt === null,
      fifthRow?.deliveryRequestedAt
    )
    ...
    check(
      'completed + deliver -> skipped',
      scored.body?.data?.emails?.team === 'skipped',
      scored.body?.data
    )
    const capturedAudit = await submitAudit(auditBody('captured', 0))
    check(
      'older captured push still lands (status advance beats stale gate)',
      capturedAudit.body?.data?.id !== null,
      capturedAudit.body?.data
    )
    const capturedRow = await readRow(auditSession)
    check(
      'status captured',
      capturedRow?.status === 'captured',
      capturedRow?.status
    )
    check(
      'lead stored',
      capturedRow?.contactEmail === auditEmail,
      capturedRow?.contactEmail
    )
    check(
      'last_activity_at did not go backwards',
      capturedRow?.lastActivityAt !== null &&
        new Date(capturedRow.lastActivityAt).getTime() >= Date.now() + 4000,
      capturedRow?.lastActivityAt
    )
    check(
      'captured + deliver -> team sent',
      capturedAudit.body?.data?.emails?.team === 'sent',
      capturedAudit.body?.data
    )
    check(
      'captured + deliver -> results sent',
      capturedAudit.body?.data?.emails?.confirmation === 'sent',
      capturedAudit.body?.data
    )
    check(
      'two audit messages in Mailpit',
      (await mailpitCount(auditTag)) === 2,
      await mailpitCount(auditTag)
    )
    check(
      'leases recorded alongside stamps',
      Boolean(
        capturedRow?.teamEmailClaimedAt &&
        capturedRow?.confirmationEmailClaimedAt
      )
    )
    ...
    const unscored = await submitAudit({
      ...
      result: null,
      lead: {
        name: `Unscored Probe ${unscoredTag}`,
        email: `audit+${unscoredTag}@example.com`,
        company: null,
        message: null,
        marketingConsent: false,
      },
    ...
    check(
      'unscored captured -> team sent',
      unscored.body?.data?.emails?.team === 'sent',
      unscored.body?.data
    )
    check(
      'unscored captured -> confirmation skipped',
      unscored.body?.data?.emails?.confirmation === 'skipped',
      unscored.body?.data
    )
    check(
      'one unscored message in Mailpit',
      (await mailpitCount(unscoredTag)) === 1,
      await mailpitCount(unscoredTag)
    )
  }
  ...
  for (const email of deliveryEmails) {
    await db
      ...
      .where(eq(rateLimitBuckets.key, `form-deliver:${email}`))
  }
  console.log('\nTest rows deleted.')
