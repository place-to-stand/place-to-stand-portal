...
| 2026-08-04 | Design revision (owner guidance) | (1) Table restyled to the projects-page pattern (bordered rounded table, muted header, visible Actions column) with filters moved inside the card; count stays in the tabs row. (2) Terminology converged on the acknowledge family — "Unacknowledged" indicator/badge/filter (URL param `unacknowledged=1`), Acknowledge/Unacknowledge actions; identifiers renamed to match. (3) New Unacknowledge action (sheet footer) with `SUBMISSION_UNACKNOWLEDGED` activity logging. (4) Detail sheet rebuilt on the house sheet pattern: fixed bottom footer, primary actions left, destructive icon-only right (Archive on List, Delete-forever on Archive); acknowledged timestamp moved into the header description. (5) Permanent delete from the Archive tab (contacts pattern: Restore + Trash2 per row, "Delete forever" ConfirmDialog, `SUBMISSION_DESTROYED` logging; query layer only destroys already-archived rows). All flows re-verified live via agent-browser + DB checks. |
