...
| D6 | **Close is an explicit action, not a calendar event.** The in-progress current month is **not closable from the UI** (post-launch owner decision); the server still allows re-close so drift fixes on an already-closed month keep working. | The common late-log case (a day or two after month end, before invoicing) costs nothing while the month is open, and closing a month halfway through makes no sense operationally. |
