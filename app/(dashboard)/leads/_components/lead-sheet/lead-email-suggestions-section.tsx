...
import type { LinkEmailSuggestedContent } from '@/lib/types/suggestions'

type LeadEmailSuggestionsSectionProps = {
  leadId: string
  isAdmin: boolean
}
...
export function LeadEmailSuggestionsSection({
  leadId,
  isAdmin,
}: LeadEmailSuggestionsSectionProps) {
  ...
  const emailSuggestions = allSuggestions.filter(s => {
    const content = s.suggestedContent as { actionType?: string }
    return content.actionType === 'LINK_EMAIL_THREAD'
  })
  ...
  if (!isAdmin || (!isLoading && emailSuggestions.length === 0)) return null

  return (
    <div className='space-y-3'>
      <div className='flex items-center gap-2'>
        <Mail className='text-muted-foreground h-4 w-4' />
        <span className='text-sm font-medium'>Email Threads</span>
      ...
      {isLoading ? (
        ...
        <div className='space-y-2'>
          {emailSuggestions.map(suggestion => {
            const content = suggestion.suggestedContent as LinkEmailSuggestedContent
            ...
            return (
              ...
              >
                ...
                <div className='flex items-center gap-2 text-xs text-muted-foreground'>
                  <span>{content.messageCount} messages</span>
                  {content.lastMessageAt && (
                    <>
                      <span>&middot;</span>
                      <span>{formatDistanceToNow(new Date(content.lastMessageAt), { addSuffix: true })}</span>
                    </>
                  )}
                </div>
